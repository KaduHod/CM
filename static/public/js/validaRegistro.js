...
//======================
      function verificaRegistro(){
        ...
        let verificaErrosUsuario = veririfcaSeInputsDeRegistroEstaCorreto(errorsMassage)
        if(verificaErrosUsuario == false){
          //boto avisos na tela
         ...
         if(window.screen.height < 790) errosBlockContainer.style.right = '-30%'
         errosBlockContainer.style.bottom = '0px'
        
        
        //====================== fim valida registro
        setTimeout(removeAvisosDeErro, 4000)
      }
      function removeAvisosDeErro(){
        let containerErros = document.getElementById('errosFlex')
        let errosBlockContainer = document.getElementById('erros')
         if(containerErros.childElementCount > 0){// retiro elementos anteriores
           ...
           if(window.screen.height < 790) errosBlockContainer.style.right = '-101%'
           else errosBlockContainer.style.bottom = '-101%'
         }
      }
      
      function veririfcaSeInputsDeRegistroEstaCorreto(errorsMassage){
        let count = null
        for(key in errorsMassage){
          if(errorsMassage[key].length > 0) count++
        } 
